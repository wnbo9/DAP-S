...
\abstract{In genome-wide association studies (GWAS), identifying causal variants from numerous genetic variants remains a fundamental challenge due to linkage disequilibrium between variants. Current genetic fine-mapping methods face multiple limitations: computational inefficiency, poor calibration of posterior probabilities, and inability to effectively incorporate functional genomic annotations. To address these challenges, we introduce DAP-PIR, a novel method that combines Deterministic Approximation of Posteriors (DAP) with Pseudo-Importance Resampling (PIR). DAP-PIR leverages variational approximation to efficiently identify a subset of high-probability causal variant combinations, while maintaining posterior estimation accuracy and substantially reducing computational costs. Through extensive simulations across diverse genetic architectures, we demonstrate that DAP-PIR achieves superior posterior probability estimation and better calibrated results compared to existing methods, including SuSiE and traditional MCMC-based fine-mapping approaches. Application to real GWAS datasets also demonstrates that DAP-PIR effectively identifies candidate causal variants while maintaining computational tractability in large-scale analyses. Key advantages of DAP-PIR include its computational efficiency, ability to dynamically update functional genomic annotations, and seamless integration with colocalization analyses. The method is implemented as an open-source R package, facilitating its adoption in genetic studies. Our results establish DAP-PIR as a robust and efficient tool for fine-mapping genetic associations, particularly valuable for modern genomic studies requiring both computational efficiency and statistical rigor.}
