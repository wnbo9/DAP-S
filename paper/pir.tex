...
\title[Article Title]{DAP-S: An Improved Variable Selection Approach for Probabilistic Fine-mapping Analysis}
...
\abstract{Probabilistic multi-SNP fine-mapping has become ubiquitous in genetic association studies. However, accurately identifying causal genetic variants in the presence of complex linkage disequilibrium (LD) remains a significant challenge. Existing computational methods, such as FINEMAP, DAP, and SuSiE, face limitations in computational efficiency, posterior probability calibration, and the ability to incorporate rapidly evolving functional genomic annotations. To address these challenges, we introduce \textbf{DAP-S}, a novel Bayesian probabilistic approach that performs a deterministic approximation of posteriors using pseudo-importance resampling of SuSiE regression results. DAP-S combines the computational efficiency of the SuSiE algorithm with several unique advantages: (1) \textbf{Interpretability}: It operates under the traditional Bayesian variable selection (BVS) model, which is more convenient to interpret than the SuSiE model. (2) \textbf{Model-level insights}: In addition to variant-level posterior inclusion probabilities (PIPs), DAP-S provides probabilistic assessments for noteworthy association models (i.e., combinations of causal SNPs), which are critical for downstream applications such as transcriptome-wide association studies (TWAS) or haplotype analysis. (3) \textbf{Flexible credible sets}: By extending the concept of signal clusters, DAP-S identifies association signals of varying strengths, enabling the construction of credible sets with arbitrary coverage probabilities. (4) \textbf{Dynamic priors}: The implementation of DAP-S allows for efficient updates of PIPs based on updated variant-level priors without requiring a complete re-run of the fine-mapping analysis. Through extensive simulations across diverse genetic architectures, we demonstrate that DAP-S produces more accurate and calibrated PIPs compared to existing methods. We also apply DAP-S to fine-map GTEx eQTL data across 48 tissues, showcasing its computational efficiency in large-scale analyses. Furthermore, we demonstrate that DAP-S outputs can be seamlessly integrated into and improve downstream TWAS and colocalization analyses.}


\keywords{Genetic fine-mapping, variable selection, variational inference, importance sampling}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{files/plot_pip_normal.pdf}
...
\begin{figure}[H]
    ...
    \caption{Power and coverage of constructed 95\% credible sets. Power is defined as the proportion of causal variants included in the credible set, and coverage is defined as the proportion of credible set containing at least one causal variants.}
    \label{fig:coverage}
\end{figure}
...
\begin{align}
    \lim _{\substack{\kappa \rightarrow 0 \\ \lambda \rightarrow 0}} \operatorname{BF}(\boldsymbol{\gamma}) &= \frac{1}{|\phi^{-2}I + \mathbf{X}^T\mathbf{X}|^{1 / 2}}  \frac{1}{\phi^{|\boldsymbol{\gamma}|}} \left[\frac{\mathbf{y}^T\left(I-\mathbf{X}\left(\phi^{-2}I+\mathbf{X}^T \mathbf{X}\right)^{-1} \mathbf{X}^T\right) \mathbf{y}}{\mathbf{y}^T \mathbf{y}}\right]^{-n / 2} \\
    &=  \frac{1}{|I + \phi^2\mathbf{X}^T\mathbf{X}|^{1 / 2}}   \left[1- \frac{\mathbf{y}^T\mathbf{X}\left(\phi^{-2}I+\mathbf{X}^T \mathbf{X}\right)^{-1} \mathbf{X}^T \mathbf{y}}{\mathbf{y}^T \mathbf{y}}\right]^{-n / 2},
\end{align}
...
Then we have:
\begin{align}
    \lim _{\substack{\kappa \rightarrow 0 \\ \lambda \rightarrow 0}} \operatorname{BF}(\boldsymbol{\gamma}) &= \frac{1}{|\mathbf{V}^{-1} + \mathbf{X}^T\mathbf{X}|^{1 / 2}} \frac{1}{\phi_1*\cdots*\phi_q} \left[\frac{\mathbf{y}^T\left(I-\mathbf{X}\left(\mathbf{V}^{-1}+\mathbf{X}^T \mathbf{X}\right)^{-1} \mathbf{X}^T\right) \mathbf{y}}{\mathbf{y}^T \mathbf{y}}\right]^{-n / 2} \\
    &=  \frac{1}{|I + \mathbf{V}\mathbf{X}^T\mathbf{X}|^{1 / 2}}   \left[1- \frac{\mathbf{y}^T\mathbf{X}\left(\mathbf{V}^{-1}+\mathbf{X}^T \mathbf{X}\right)^{-1} \mathbf{X}^T \mathbf{y}}{\mathbf{y}^T \mathbf{y}}\right]^{-n / 2}.
\end{align}
