...
Additionally, the Root Mean Squared Error (RMSE) of the posterior inclusion probabilities (PIPs) from the DAP-PIR algorithm is smaller than that of both the DAP-G and SuSiE algorithms, demonstrating better posterior approxiamtion. Calibration plots (Figures \ref{fig11}-\ref{fig51}) further confirm that the DAP-PIR algorithm yields more accurate PIP estimates for each vairant than the other methods.
