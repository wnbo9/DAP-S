...
//' @export
// [[Rcpp::export]]
List dap_susie_main(NumericMatrix X, 
              NumericVector y,
              NumericMatrix matrix,
              NumericVector prior_weights,
              double r2_threshold,
              double coverage,
              bool use_susie_variance_estimate,
              NumericVector phi2_vec,
              bool greedy,
              double pir_threshold) {
    if (use_susie_variance_estimate) {
        Rcout << "Use prior effect size variance estimated from SuSiE: " << use_susie_variance_estimate << std::endl;
    } else {
        Rcout << "Using a grid value of phi2: " << phi2_vec << std::endl;
    }
    Rcout << "Greedy resampling: " << greedy << std::endl;
    ...
    List sc_results = get_sc(X, matrix, cmfg_mat, posterior_prob, col_names, pir_threshold, r2_threshold, coverage);
